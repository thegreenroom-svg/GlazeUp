...
export default function NeedsVerificationPage() {
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 800, margin: '0 auto' }}>
      <h1 style={{ fontSize: 'var(--text-xl)', fontWeight: 700, marginBottom: '0.3rem' }}>Bookings Needing Verification</h1>
      <p style={{ color: '#666', fontSize: 'var(--text-base)', marginBottom: '1rem' }}>
      ...
      <button
        onClick={load}
        disabled={loading}
        style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1rem', backgroundColor: 'var(--clay)', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: 'var(--text-base)', marginBottom: '1.25rem' }}
      >
            ...
            return (
              <div key={b.booking_code} style={{ padding: '0.9rem', border: `1px solid ${upcoming ? '#e0c060' : '#ddd'}`, backgroundColor: upcoming ? '#fffdf5' : 'white', borderRadius: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem' }}>
                  ...
                  {upcoming && <span style={{ fontSize: 'var(--text-xs)', color: '#8a5a00', fontWeight: 700 }}>UPCOMING</span>}
                </div>
                <p style={{ fontSize: 'var(--text-base)', color: '#333' }}>
                  {start.toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'short' })}
                  {' · '}
                  {start.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                  {b.table_number ? ` · Table ${b.table_number}` : ''}
                </p>
                <p style={{ fontSize: 'var(--text-sm)', color: '#999', marginTop: '0.3rem' }}>{b.notes}</p>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.3rem' }}>
                  <p style={{ fontSize: 'var(--text-xs)', color: '#bbb', fontFamily: 'monospace' }}>{b.booking_code}</p>
                  <a href={`/bookings?code=${b.booking_code}`} style={{ fontSize: 'var(--text-xs)', color: 'var(--clay)', fontWeight: 600, textDecoration: 'none' }}>
                    Open booking →
                  </a>
                </div>
              </div>
